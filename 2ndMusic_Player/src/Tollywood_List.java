...
public class Tollywood_List extends JFrame{
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					Tollywood_List window = new Tollywood_List();
	 ...
	 */
	public Tollywood_List() {
		Tframe= new JFrame();
		Tframe.setBounds(500, 300, 861, 532);
		Tframe.getContentPane().setBackground(SystemColor.inactiveCaptionBorder);
		...
		Tframe.getContentPane().add(pnlHeader);
		...
		Tframe.getContentPane().add(pnlHeaderTrack);
		...
		Tframe.getContentPane().add(pane);
		...
		try
		{
		    ...
		    Connection con = DriverManager.getConnection(url, "root", "rutu2012089");
              ...
              String email=AccessEmail.getEmailId();
    			String query = "select song,song_name,artist from songs_list where caterory = 'Tollywood' and email='"+email+"' ";
